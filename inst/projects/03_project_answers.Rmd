...
counts(dds) %>% 
  as_tibble() %>% 
  select(sample1) %>% 
  ggplot(aes(log2(sample1 + 1))) + # why +1? Because log2(0) is not an option...
...
# Some stats here...
# What is this distribution? Because it is clearly non normal... So, "classical" tests may not work here... WE WILL NEED SPECIFIC TESTS
```
