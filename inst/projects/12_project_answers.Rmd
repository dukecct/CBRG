...
# inspect results
exprs(st000336)
summary(exprs(st000336))

glimpse(exprs(st000336)) # doesn't quite work
exprs(st000336) %>% 
  as_tibble() %>% 
  glimpse()
...
identical(dim(exprs(st000336)), dim(exprs(imputed))) # no
...
exprs(imputed) %>% 
  ...
  pivot_longer(cols = everything(),
               names_to = "sample",
               values_to = "intensity") %>% 
  ggplot(aes(intensity, colour = sample)) +
  geom_boxplot() +
...
# old-fashioned way
df <- exprs(pre_processed)
groups <- as.factor(pre_processed$Group)
design <- model.matrix(~groups)
fit <- lmFit(df, design)
fit <- eBayes(fit)
topTable(fit, number = Inf) %>% View()
```
