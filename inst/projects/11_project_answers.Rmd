...
res_sum %>%
    ...
    geom_vline(xintercept = c(-1, 1)) +
    geom_hline(yintercept = -log10(0.05)) +
    scale_color_manual(values = c("black","red")) # more true positives are detected but LOOK AT THE FOLD-CHANGES!!!
...
res_mean %>%
    ...
    geom_vline(xintercept = c(-1, 1)) +
    geom_hline(yintercept = -log10(0.05)) +
    scale_color_manual(values = c("black","red")) # slightly better than median aggregation
...
res_med_pol %>%
    ...
    geom_vline(xintercept = c(-1, 1)) +
    geom_hline(yintercept = -log10(0.05)) +
    scale_color_manual(values = c("black","red")) # Amazing!!
...
res_rob_sum %>%
    ...
    geom_vline(xintercept = c(-1, 1)) +
    geom_hline(yintercept = -log10(0.05)) +
    scale_color_manual(values = c("black","red")) # 21 TPs detected!
```
...
# make design table
cond <- as.factor(prots$condition)
...
# DEqMS part analysis
psm.count.table = as.data.frame(table(cptac_psm$gene))
