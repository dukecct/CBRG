...
table(nci_labs) %>% 
  as_tibble() %>%
  ggplot(aes(reorder(nci_labs, -n), n)) +
...
nci_small <- nci_data %>% 
  janitor::clean_names() %>% 
  dplyr::select_at(vars(contains("breast") | contains("colon")))
```
...
colnames(nci_top_50_scaled) # we just need to stip off the "_" and numbers
...
# BONUS - Visualize the scaling effect on a PCA with POMA
...
not_normalized <- st000284 %>% 
  PomaMultivariate(method = "pca")

normalized <- st000284 %>% 
  PomaNorm(method = "log_scaling") %>% # note that this line is the only difference
  PomaMultivariate(method = "pca")

# while the sample separation in the 2D space doesn't change a lot, 
# the variance % explained for the first component does
not_normalized$scoresplot + ggtitle("Not normalized") | normalized$scoresplot + ggtitle("Normalized") 

# why?
# Because in the left side of the plot (not normalized) the variance of almost all dataset is explained by few variables 
# (those with more variance - not scaled), and in the right side, the explained variance is lower, 
# but distributed across features

(not_normalized$screeplot + 
    ggtitle("Not normalized") +
    theme(legend.position = "none") | 
    normalized$screeplot + 
    ggtitle("Normalized") + 
    theme(legend.position = "none")) /
  (not_normalized$loadings_plot + 
     theme(legend.position = "top") | 
     normalized$loadings_plot + 
     theme(legend.position = "top"))
```
