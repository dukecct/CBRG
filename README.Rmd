...
To launch a presentation for a particular meetup (e.g. meetup 1), run the following 
``` r
...
```
To see the corresponding file with ALL the code, set the `show_answers = ` parameter to `TRUE`
``` r
CBRG::launch_project(1, show_answers = TRUE)
```
To launch the homework R Markdown files, use the function `launch_exercise()` (which works just like `launch_project()`)
...
and 
[Omics Data Analysis](https://uclouvain-cbio.github.io/WSBIM2122/index.html) by Prof. Laurent Gatto  
